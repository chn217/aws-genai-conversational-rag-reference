...
import { DocumentMetadata, DocumentMetadataLoaderError, DocumentValidationError } from '../../../lib/types';
import { documentUploadCommandFlags } from '../flags';

export default class DocumentUploadCommand extends Command {
  static summary = "Uploads documents based on passed metadata to the application's S3 bucket";
  ...
  async run(): Promise<void> {
      ...
      await prompts(
        [
          galileoPrompts.filePathPrompt({
            ...
            what: `${chalk.yellowBright('metadata file')} or the ${chalk.yellowBright(
    ...
    try {
      ...
      if (err instanceof DocumentValidationError) {
        context.ui.spinner.fail(`Error validating metadata: ${chalk.redBright(err.message)}. Quitting...`);
      } else if (err instanceof DocumentMetadataLoaderError) {
        context.ui.spinner.fail(
          `Error while loading metadata with loader: ${chalk.redBright(err.message)}. Quitting...`,
        );
      } else {
      ...
      const { confirmed } = context.cachedAnswers(
        await prompts(
          galileoPrompts.confirmExec({
            message: `Confirm to upload ${Object.keys(documentMetadata!.documents).length} documents?`,
          }),
        ),
      );
